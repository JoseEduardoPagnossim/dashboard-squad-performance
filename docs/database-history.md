> **Documento histórico.** Para instalação ou manutenção da versão atual, use [`docs/database.md`](database.md) e `supabase/schema.sql`.
...
Execute `supabase/migrations/MIGRACAO_V2.29.6.sql` após a V2.29.5. Ela não cria colunas novas; apenas recria `get_my_squad_game_ranking(date,date)` com as regras consolidadas.
...
Antes de publicar o frontend, execute `supabase/migrations/MIGRACAO_V2.26.0.sql`. A migração cria `quality_daily_metrics`, usada exclusivamente para as avaliações de **Produto** e **Empresa**.
...
Antes de publicar o frontend, execute `supabase/migrations/MIGRACAO_V2.21.0.sql`. A migração adiciona as notas 4, 3, 2 e 1 em `daily_metrics` e recria os RPCs agregados diários usados nas análises.
...
1. `supabase/migrations/MIGRACAO_V2.19.0.sql`;
2. deploy da `manage-user` V2.19;
3. deploy da `create-user` V2.19;
4. frontend V2.19;
5. logout/login e hard refresh.

Não recrie banco nem rode `supabase/schema.sql` sobre uma produção já existente. O schema completo é para instalação nova.

## 10. Instalação nova

1. execute `supabase/schema.sql`;
2. crie o primeiro Admin Geral no Authentication;
3. execute `supabase/bootstrap_primeiro_admin.sql` com o UUID;
4. publique `create-user` e `manage-user`;
5. configure `js/config.js` com URL e anon/publishable key;
...
Execute `supabase/migrations/MIGRACAO_V2.20.0.sql` após a V2.19.0.
...
A V2.22.0 altera somente HTML, JavaScript de interação do seletor de data e CSS visual. Não cria colunas, tabelas, políticas ou funções novas no Supabase. Se a `supabase/migrations/MIGRACAO_V2.21.0.sql` já foi executada, não existe SQL adicional para esta versão.


## V2.25.1 — Feedbacks mensais

Execute `supabase/migrations/MIGRACAO_V2.25.1.sql` antes de publicar o frontend. A migração cria/valida `technician_feedbacks` e pode ser executada novamente com segurança.
...
```sql
-- conteúdo do arquivo supabase/migrations/MIGRACAO_V2.27.0.sql
```
...
Execute `supabase/migrations/MIGRACAO_V2.28.0.sql` antes de publicar a V2.28.0.
...
Execute `supabase/migrations/MIGRACAO_V2.29.0.sql` antes de publicar a V2.29.0.
...
A V2.29.1 adiciona somente cálculos e visualizações estratégicas sobre `quality_financial_monthly`. Não há tabela, coluna, função ou política RLS nova. Se a `supabase/migrations/MIGRACAO_V2.29.0.sql` já foi executada, não é necessário rodar SQL adicional.


## V2.29.2 — atualização do ranking/status

Execute `supabase/migrations/MIGRACAO_V2.29.2.sql` após a V2.20.5/V2.24.2.
...
Execute `supabase/migrations/MIGRACAO_V2.29.5.sql` após a V2.29.2.
