...
Durante a modernização dos gráficos da V2.23.0, a função `buildStatusMatrix()` foi removida acidentalmente do `js/app.js`, embora `renderTechnicianStatusMatrix()` continuasse chamando essa função.
...
1. Preserve o `js/config.js` atual do ambiente de produção.
