...
Execute uma vez `supabase/migrations/MIGRACAO_V2.13.0.sql` no SQL Editor. A função retorna somente métricas operacionais necessárias ao comparativo. Não expõe e-mail, senha, bônus, desconto ou dados de autenticação.

Depois, publique `index.html`, `js/app.js` e `css/styles.css`, mantendo o `js/config.js` atual.
