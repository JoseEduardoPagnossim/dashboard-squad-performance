...
Se a V2.1.1 já está funcionando no mesmo projeto Supabase, **não execute novamente o `supabase/schema.sql`**. A V2.2.0 utiliza as mesmas tabelas.

## Antes de atualizar o GitHub

Guarde o conteúdo do seu `js/config.js` atual.
...
Depois confira se o `js/config.js` continua com as credenciais do seu projeto.
