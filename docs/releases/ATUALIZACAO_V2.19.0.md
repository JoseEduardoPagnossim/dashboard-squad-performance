...
- guarde seu `js/config.js` publicado em local seguro;
- opcionalmente exporte/registre as configurações atuais do Supabase.

**Não substitua o `js/config.js` que já funciona em produção.**
...
```text
supabase/migrations/MIGRACAO_V2.19.0.sql
```
...
```text
dashboard-squad-performance-v2.19.0-atualizacao-github.zip
```

Substitua os arquivos equivalentes no repositório.

O pacote de atualização **não contém `js/config.js`**. Mantenha exatamente o arquivo que já está conectado ao seu projeto Supabase.
...
- Erro ao salvar modelo financeiro: confirme `supabase/migrations/MIGRACAO_V2.19.0.sql`.
