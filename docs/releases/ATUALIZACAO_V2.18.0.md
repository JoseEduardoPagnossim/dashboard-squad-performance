...
Execute **uma vez** `supabase/migrations/MIGRACAO_V2.18.0.sql` no SQL Editor do Supabase. A migração adiciona as estruturas financeiras e não apaga dados existentes.

## Publicação

1. Preserve o `js/config.js` já configurado no GitHub.
2. Envie os arquivos do ZIP `dashboard-squad-performance-v2.18.0-atualizacao-github.zip`.
