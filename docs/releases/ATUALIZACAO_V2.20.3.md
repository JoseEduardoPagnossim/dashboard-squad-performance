...
Não há SQL novo e não é necessário republicar Edge Functions. Substitua os arquivos do frontend, preserve seu `js/config.js`, faça logout/login e `Ctrl + F5`. Depois reimporte a competência aberta para reconstruir os dados com todos os técnicos que possuem registros no CSV.
