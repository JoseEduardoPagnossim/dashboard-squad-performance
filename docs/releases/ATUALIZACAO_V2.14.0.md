...
Execute uma vez `supabase/migrations/MIGRACAO_V2.14.0.sql`. Ela cria a RPC `get_org_daily_attendance_overview`, que retorna somente totais agregados por Squad/dia para usuários autenticados da mesma organização. Nenhum dado individual é exposto por essa função.

## Publicação

Mantenha o `js/config.js` que já está configurado no ambiente publicado e substitua os demais arquivos do pacote de atualização.
