(() => {
  ...
  async function renderAuditLogView(force=false){if(!isAdmin())return;if($('#auditScopeText'))$('#auditScopeText').textContent=isSuperAdmin()?'Você visualiza a auditoria de toda a organização. Admins de Squad visualizam apenas o próprio escopo.':`Você visualiza apenas as ações administrativas do Squad ${state.user.squadCode}.`;if($('#auditStatus'))$('#auditStatus').textContent='Carregando auditoria...';await loadAuditLogs(force);renderAuditRows();}
